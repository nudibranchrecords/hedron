  ...
  const onElementScrub = useCallback(
    ({ x, y }: { x: number; y: number }) => {
      if (isVector2) {
        ...
        const paramValues = storeState.paramValues
        const updateParamValue = storeState.updateParamValue

        const vec2 = [x, y]

        selectedNode.childGroups.vectorComponentIds.forEach((id, i) => {
          const oldVal = paramValues[id] as number
          const sliderMin = (paramValues[`${id}-sliderMin`] as number | undefined) ?? 0
          const sliderMax = (paramValues[`${id}-sliderMax`] as number | undefined) ?? 1
