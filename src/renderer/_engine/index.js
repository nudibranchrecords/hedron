...
export const reloadSingleSketchModule = (url, moduleId, pathArray) => {
  try {
    ...
    console.error(error)
    store.dispatch(
      projectError(`Sketch ${moduleId} failed to load: ${error.message}`, {
        popup: 'true',
        code: error.code,
      }),
    )
...
export const reloadSingleSketchConfig = (url, moduleId, pathArray) => {
  try {
    ...
    console.error(error)
    store.dispatch(
      projectError(`Sketch config ${moduleId} failed to load: ${error.message}`, {
        popup: 'true',
        code: error.code,
      }),
    )
  ...
  const updateSceneSketches = (sceneId) => {
    ...
    if (stateScene) {
      stateScene.sketchIds.forEach((sketchId) => {
        ...
        if (sketch.update && !stateScene.settings.globalPostProcessingEnabled)
          sketch.update({ ...getInfo(), params })
      })
    }
  }
